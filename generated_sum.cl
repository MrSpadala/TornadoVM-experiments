...
__kernel void reductionAddFloats(__global uchar *_heap_base, ulong _frame_base, __constant uchar *_constant_region, __local uchar *_local_region, __global uchar *_private_region)
{
  ...
  *((__global float *) ul_2)  =  0.0F;
  ...
  // BLOCK 1 MERGES [0 7 ]
  i_4  =  i_3;
  for(;i_4 < 8192;)  {  //actually this for is executed only once. It acts like an if statement
    // BLOCK 2
    i_5  =  get_local_id(0);
    i_6  =  get_local_size(0);
    i_7  =  get_group_id(0);
    i_8  =  i_6 * i_7;  
    i_9  =  i_8 + i_5;   //i_9 contains the offset in work-items from the base
    l_10  =  (long) i_9;
    l_11  =  l_10 << 2;  //l_11 contains the offset in bytes from the base (4 bytes per float)
    l_12  =  l_11 + 24L;
    ul_13  =  ul_0 + l_12;  //ul_13 is the address in which the thread will write its sum
    i_14  =  i_6 >> 31;
    i_15  =  i_14 >> 31;
    ...
    i_18  =  i_17;
    for(;i_18 >= 1;) {
      // Reduction. Loop until i_18 is greater or equal than 1. At every iteration divide i_18 by 2.
      // Effectively what the code is doing is a reduction at work-group level. On the first
      // iteration half the elements are summed with the other half. At the second iteration
      // half of the already summed elements are summed with the second half (quarters of the initial array).
      // When all the elements are summed, i_18 is lower than 1 and this loop exits.
      // BLOCK 8
      ...
      if(z_23)   //the local_id is less than i_18?
      {
        ...
        i_25  =  i_9 + i_18;  //at the first iteration i_18 is half the local size. i_25 contains the offset from the base in work-items of the second float to sum
        l_26  =  (long) i_25;
        l_27  =  l_26 << 2;
        l_28  =  l_27 + 24L;
    ...
    i_34  =  i_33 + i_4;
    z_35  =  i_5 == 0;  //z_35 is true if the thread is assigned to the first item in the work group
    if(z_35)
    {
